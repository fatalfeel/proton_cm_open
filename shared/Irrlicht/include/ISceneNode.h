	...
	*/
	...
	public:
		...
		*/
        ...
        //by stone
		void setObjectMaterialTexture(u32 objectid, u32 textureLayer, video::ITexture* texture)
		{
			if (textureLayer >= video::MATERIAL_MAX_TEXTURES)
				return;
            
			if( objectid < getMaterialCount() )
				getMaterial(objectid).setTexture(textureLayer, texture);
		}
		...
		//by stone
		void setObjectMaterialType(u32 objectid, video::E_MATERIAL_TYPE newType)
		{
			if( objectid < getMaterialCount() )
				getMaterial(objectid).MaterialType = newType;
		}
