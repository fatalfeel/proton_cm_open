...
CL_UnknownSharedPtr CL_ThreadLocalStorage_Impl::get_variable(const std::string &name)
{
	//std::map<CL_String, CL_UnknownSharedPtr>::const_iterator it = data.find(name);
    std::map<std::string, CL_UnknownSharedPtr>::const_iterator it = data.find(name);
...
void CL_ThreadLocalStorage_Impl::set_variable(const std::string &name, CL_UnknownSharedPtr ptr)
{
	data[name] = ptr;
}
