...
//by stone
typedef struct _zipmemory_s
{
	unsigned char*	base; /* Base of the region of memory we're using */
...
/////////////////////////by stone
extern zipFile ZEXPORT zipOpen2_mem (buf, size, append, globalcomment, pzlib_filefunc_def)
    unsigned char*		buf;
    int					size;
	int					append;
    zipcharpc*			globalcomment;
    zlib_filefunc_def*	pzlib_filefunc_def;
{
	...
	fill_memory_filefunc(&ziinit.z_filefunc);
	
	ziinit.filestream = (*(ziinit.z_filefunc.zopen_mem))(ziinit.z_filefunc.opaque,
														 buf,
														 size);
...
extern zipFile ZEXPORT ZipOpen_Memory (buf, size, append)
    unsigned char* buf; 
	int size;
    int append;
{
    return zipOpen2_mem(buf,size,append,NULL,NULL);
}
