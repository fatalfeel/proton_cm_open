...
namespace boost 
{
  namespace BOOST_SIGNALS_NAMESPACE 
  {
    namespace detail 
	{
	  ...
	  {
        BOOST_SIGNALS_ARGS_STRUCT(BOOST_SIGNALS_COPY_PARMS) BOOST_SIGNALS_INIT_ARGS
        {
        }
      ...
      struct BOOST_SIGNALS_CALL_BOUND 
	  {
        ...
        struct caller 
		{
          typedef BOOST_SIGNALS_ARGS_STRUCT<BOOST_SIGNALS_TEMPLATE_ARGS> *args_type;
  ...
  template<	typename R,
			...
			typename SlotFunction = BOOST_SIGNALS_FUNCTION<	R BOOST_SIGNALS_COMMA_IF_NONZERO_ARGS
															BOOST_SIGNALS_TEMPLATE_ARGS > >class BOOST_SIGNALS_SIGNAL
...
public:
  ...
  private:
    ...
    typedef BOOST_SIGNALS_NAMESPACE::detail
	::BOOST_SIGNALS_CALL_BOUND<R> outer_bound_slot_caller;
    
	typedef typename outer_bound_slot_caller
	::template caller<	BOOST_SIGNALS_TEMPLATE_ARGS
						BOOST_SIGNALS_COMMA_IF_NONZERO_ARGS
						slot_function_type	> call_bound_slot;
  ...
  public:
    ...
    typedef BOOST_SIGNALS_NAMESPACE::detail
	::slot_call_iterator<call_bound_slot, iterator>slot_call_iterator;

    explicit
    BOOST_SIGNALS_SIGNAL(const Combiner& c = Combiner(), const GroupCompare& comp = GroupCompare())
	: BOOST_SIGNALS_NAMESPACE::detail::signal_base(real_group_compare_type(comp), c)
    {
    }

    // Connect a slot to this signal
    BOOST_SIGNALS_NAMESPACE::connection
    connect(const slot_type&, BOOST_SIGNALS_NAMESPACE
	::connect_position at = BOOST_SIGNALS_NAMESPACE::at_back);


    BOOST_SIGNALS_NAMESPACE::connection
    connect(const group_type&, const slot_type&, BOOST_SIGNALS_NAMESPACE
	::connect_position at = BOOST_SIGNALS_NAMESPACE::at_back);
  ...
  BOOST_SIGNALS_NAMESPACE::connection
  BOOST_SIGNALS_SIGNAL<	R, 
						BOOST_SIGNALS_TEMPLATE_ARGS
						BOOST_SIGNALS_COMMA_IF_NONZERO_ARGS
						Combiner, 
						Group, 
						GroupCompare, 
						SlotFunction >::connect(const slot_type& in_slot, 
  ...
  BOOST_SIGNALS_SIGNAL<	R, 
						...
						Group, 
						GroupCompare, 
						SlotFunction >::connect(const group_type& group,
  ...
  template<	typename R,
			...
			typename GroupCompare,
			typename SlotFunction >
  typename BOOST_SIGNALS_SIGNAL<R, 
								BOOST_SIGNALS_TEMPLATE_ARGS
								BOOST_SIGNALS_COMMA_IF_NONZERO_ARGS
								Combiner, 
								Group, 
  ...
  BOOST_SIGNALS_SIGNAL< R, 
						...
						Group, 
						GroupCompare, 
						SlotFunction>::operator()(BOOST_SIGNALS_PARMS)
  {
  ...
  BOOST_SIGNALS_SIGNAL<	R, 
						...
						Group, 
						GroupCompare, 
						SlotFunction >::operator()(BOOST_SIGNALS_PARMS) const
  {
    // Notify the slot handling code that we are making a call
    BOOST_SIGNALS_NAMESPACE::detail::call_notification notification(this->impl);
