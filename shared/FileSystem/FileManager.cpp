...
{
	Kill();
	m_pData = FileManager::GetFileManager()->Get(fileName, &m_size, bAddBasePath);

	return IsLoaded();
}

//by jesse stone
static FileManager* file_manager_instance = NULL;
...
void FileManager::Free()
{
	if( file_manager_instance )
	{
		delete file_manager_instance;
		file_manager_instance = NULL;
	}
}
...
byte* FileManager::Get( string fileName, int *pSizeOut, bool bAddBasePath, bool bAutoDecompress)
{
	...
	{
		//(*itor)->GetDeliveryTime() > m->GetDeliveryTime())
		pData = (*itor)->Get_unz(fileName, pSizeOut);
		
        if (pData)
            break; //guess we found it
