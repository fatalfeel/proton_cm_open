...
bool StreamingInstanceZip::OpenFileAtCurrentLocation()
{
	...
	int err = unzGetCurrentFileInfo(m_unzf,&m_unzfile_info,st_filename_inzip,sizeof(st_filename_inzip),NULL,0,NULL,0);
...
bool StreamingInstanceZip::Open( string fName)
{
	...
	if (unzLocateFile(m_unzf,(m_rootDir+fName).c_str(),CASESENSITIVITY)!=UNZ_OK)
	{
		return false;
	}
...
int StreamingInstanceZip::Read( byte * pBufferOut, int maxBytesToRead )
{
	...
	while(maxBytesToRead >= 0)
	{
		...
		int err = unzReadCurrentFile(m_unzf,m_buffer,BUFFER_SIZE);
		if (err<0)	
		{
...
void StreamingInstanceZip::Close()
{
	if (!m_bIsFinished)
	{
		int err = unzCloseCurrentFile(m_unzf);
		if (err!=UNZ_OK)
		{
...
void StreamingInstanceZip::SeekFromStart( int byteCount )
{
	...
	int offset = unzGetRawFilePos(m_unzf)+byteCount;
	//since we moved, a readcache won't make sense anymore if it existed

#ifdef _DEBUG
	char filename_inzip[512];
	unz_file_info file_info;
	unzGetCurrentFileInfo(m_unzf,&file_info,filename_inzip,sizeof(filename_inzip),NULL,0,NULL,0);
