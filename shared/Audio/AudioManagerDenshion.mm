...
//by jesse stone
static AudioManagerDenshion* audio_denshion_manager_instance = NULL;

AudioManagerDenshion* AudioManagerDenshion::GetAudioManager()
{
	if (!audio_denshion_manager_instance)
		audio_denshion_manager_instance = new AudioManagerDenshion;
	return audio_denshion_manager_instance;
}

void AudioManagerDenshion::Free()
{
	if( audio_denshion_manager_instance )
	{
		delete audio_denshion_manager_instance;
		audio_denshion_manager_instance = NULL;
	}
}
...
AudioHandle AudioManagerDenshion::Play( string fName, bool bLooping /*= false*/, bool bIsMusic /*= false*/, bool bAddBasePath, bool bForceStreaming )
{
	...
	if (bIsMusic)
	{
		...
		{
			LogMsg("Error: Can't locate audio file %s!", fNameTemp.c_str());
			return AUDIO_HANDLE_BLANK;
		}
		if (m_bgMusicPlayer)
		{
...
void AudioManagerDenshion::Update()
{
	...
	{
		unsigned int now = BaseApp::GetBaseApp()->GetGameTick();
...
void AudioManagerDenshion::FadeOutMusic(unsigned int duration)
{
	...
	m_musicFadeOutStartTime = BaseApp::GetBaseApp()->GetGameTick();
