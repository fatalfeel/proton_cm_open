...
//by jesse stone
static AudioManagerAndroid* audio_android_manager_instance = NULL;

AudioManagerAndroid* AudioManagerAndroid::GetAudioManager()
{
	if (!audio_android_manager_instance)
		audio_android_manager_instance = new AudioManagerAndroid;
	return audio_android_manager_instance;
}

void AudioManagerAndroid::Free()
{
	if( audio_android_manager_instance )
	{
		delete audio_android_manager_instance;
		audio_android_manager_instance = NULL;
	}
}
...
AudioHandle AudioManagerAndroid::Play( string fName, bool bLooping /*= false*/, bool bIsMusic, bool bAddBasePath, bool bForceStreaming)
{
	...
	if (env)
	{
		...
		if (streamID == 0)
		{
			...
			MessageManager::GetMessageManager()->SendGame(MESSAGE_TYPE_PLAY_SOUND, fName, 50, TIMER_SYSTEM);
			return (AudioHandle) 0;
		}
