...
void *
xmlMallocLoc(unsigned int size, const char * file, int line)
{
    MEMHDR *p;
    void *ret;
...
void *
xmlMallocAtomicLoc(unsigned int size, const char * file, int line)
{
    MEMHDR *p;
    void *ret;
...
xmlMemFree(void *ptr)
{
    ...
    if (ptr == (void *) -1) {
	xmlGenericError(xmlGenericErrorContext,
	    "trying to free pointer from kdFreed area\n");
        goto error;
    }
