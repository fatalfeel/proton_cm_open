...
#ifndef SIZE_T_MAX
# define SIZE_T_MAX     ((unsigned int)-1)
#endif /* !SIZE_T_MAX */
             ...
             const xmlChar *value, const xmlChar *prefix ATTRIBUTE_UNUSED)
{
    ...
    ret = xmlNewNsPropEatName(ctxt->node, _namespace, name, NULL);
...
xmlSAX2Characters(void *ctx, const xmlChar *ch, int len)
{
	...
	if ((coalesceText) && (ctxt->nodemem != 0)) {
		...
		lastChild->content = xmlStrdup(lastChild->content);
            ...
            if (((unsigned int)ctxt->nodelen + (unsigned int)len > XML_MAX_TEXT_LENGTH) &&
                ((ctxt->options & XML_PARSE_HUGE) == 0)) {
                xmlSAX2ErrMemory(ctxt, "xmlSAX2Characters: huge text node");
                return;
            }
	    if ((unsigned int)ctxt->nodelen > SIZE_T_MAX - (unsigned int)len || 
	        (unsigned int)ctxt->nodemem + (unsigned int)len > SIZE_T_MAX / 2) {
