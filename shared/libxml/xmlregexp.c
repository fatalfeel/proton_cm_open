...
xmlFARegExec(xmlRegexpPtr comp, const xmlChar *content) {
	...
	for (;exec->transno < exec->state->nbTrans;exec->transno++) {
		...
		printf("epsilon transition left at runtime\n");
		exec->status = -2;
		break;
...
xmlAutomataNewNegTrans(xmlAutomataPtr am, xmlAutomataStatePtr from,
    ...
    win_snprintf ((char *) err_msg, 199, "not %s", (const char *) atom->valuep);
    err_msg[199] = 0;
...
xmlExpDumpInt(xmlBufferPtr buf, xmlExpNodePtr expr, int glob) {
    ...
    switch (expr->type) {
		...
		rep[1] = 0;
	        ...
	        win_snprintf (rep, 39, "{%d}", expr->exp_min);
	    } else if (expr->exp_max < 0) {
	        win_snprintf (rep, 39, "{%d,inf}", expr->exp_min);
	    } else {
	        win_snprintf (rep, 39, "{%d,%d}", expr->exp_min, expr->exp_max);
	    }
	    rep[39] = 0;
