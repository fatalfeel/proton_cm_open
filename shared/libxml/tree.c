...
xmlGetNodePath(xmlNodePtr node)
{
    ...
    do {
		...
		if (cur->ns->prefix != NULL) {
                    win_snprintf (nametemp, sizeof(nametemp) - 1, "%s:%s",
	    ...
	    */
            ...
            sep = "/";
	    win_snprintf (nametemp, sizeof(nametemp) - 1,
	    ...
	    next = cur->parent;
		    ...
		    win_snprintf (nametemp, sizeof(nametemp) - 1, "%s",
			(char *)cur->name);
                nametemp[sizeof(nametemp) - 1] = 0;
                name = nametemp;
            }
        ...
        if (occur == 0)
            win_snprintf ((char *) buf, buf_len, "%s%s%s",
                     sep, name, (char *) buffer);
        else
            win_snprintf ((char *) buf, buf_len, "%s%s[%d]%s",
                     sep, name, occur, (char *) buffer);
        win_snprintf ((char *) buffer, buf_len, "%s", (char *)buf);
        cur = next;
    } while (cur != NULL);
    xmlFree(buf);
    return (buffer);
...
xmlNewReconciliedNs(xmlDocPtr doc, xmlNodePtr tree, xmlNsPtr ns) {
     ...
     */
    if (ns->prefix == NULL)
	win_snprintf ((char *) prefix, sizeof(prefix), "default");
    else
	win_snprintf ((char *) prefix, sizeof(prefix), "%.20s", (char *)ns->prefix);

    def = xmlSearchNs(doc, tree, prefix);
    while (def != NULL) {
        if (counter > 1000) return(NULL);
	if (ns->prefix == NULL)
	    win_snprintf ((char *) prefix, sizeof(prefix), "default%d", counter++);
	else
	    win_snprintf ((char *) prefix, sizeof(prefix), "%.20s%d",
 ...
 */
xmlBufferPtr
xmlBufferCreateSize(unsigned int size) {
    xmlBufferPtr ret;
 ...
 */
xmlBufferPtr
xmlBufferCreateStatic(void *mem, unsigned int size) {
    xmlBufferPtr ret;
...
ns_next_prefix:
	...
	if (counter > 1000)
	    return (NULL);
	if (prefix == NULL) {
	    win_snprintf ((char *) buf, sizeof(buf),
		"ns_%d", counter);
	} else
	    win_snprintf ((char *) buf, sizeof(buf),
