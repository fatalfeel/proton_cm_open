                         ...
                         const xmlChar * arg2)
{
    ...
    switch (err) {
        ...
        case XML_RELAXNG_ERR_TYPE:
            win_snprintf (msg, 1000, "failed to validate type %s\n", arg1);
            break;
        case XML_RELAXNG_ERR_TYPEVAL:
            win_snprintf (msg, 1000, "Type %s doesn't allow value '%s'\n", arg1,
                     arg2);
            break;
        case XML_RELAXNG_ERR_DUPID:
            win_snprintf (msg, 1000, "ID %s redefined\n", arg1);
            break;
        case XML_RELAXNG_ERR_TYPECMP:
            win_snprintf (msg, 1000, "failed to compare type %s\n", arg1);
            break;
        ...
        case XML_RELAXNG_ERR_INTERNAL:
            win_snprintf (msg, 1000, "Internal error: %s\n", arg1);
            break;
        case XML_RELAXNG_ERR_LISTEXTRA:
            win_snprintf (msg, 1000, "Extra data in list: %s\n", arg1);
            break;
        ...
        case XML_RELAXNG_ERR_INTEREXTRA:
            win_snprintf (msg, 1000, "Extra element %s in interleave\n", arg1);
            break;
        case XML_RELAXNG_ERR_ELEMNAME:
            win_snprintf (msg, 1000, "Expecting element %s, got %s\n", arg1,
                     arg2);
            break;
        case XML_RELAXNG_ERR_ELEMNONS:
            win_snprintf (msg, 1000, "Expecting a namespace for element %s\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_ELEMWRONGNS:
            win_snprintf (msg, 1000,
                     "Element %s has wrong namespace: expecting %s\n", arg1,
                     arg2);
            break;
        case XML_RELAXNG_ERR_ELEMWRONG:
            win_snprintf (msg, 1000, "Did not expect element %s there\n", arg1);
            break;
        ...
        case XML_RELAXNG_ERR_ELEMEXTRANS:
            win_snprintf (msg, 1000, "Expecting no namespace for element %s\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_ELEMNOTEMPTY:
            win_snprintf (msg, 1000, "Expecting element %s to be empty\n", arg1);
            break;
        case XML_RELAXNG_ERR_NOELEM:
            win_snprintf (msg, 1000, "Expecting an element %s, got nothing\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_NOTELEM:
            return (xmlCharStrdup("Expecting an element got text\n"));
        case XML_RELAXNG_ERR_ATTRVALID:
            win_snprintf (msg, 1000, "Element %s failed to validate attributes\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_CONTENTVALID:
            win_snprintf (msg, 1000, "Element %s failed to validate content\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_EXTRACONTENT:
            win_snprintf (msg, 1000, "Element %s has extra content: %s\n",
                     arg1, arg2);
            break;
        case XML_RELAXNG_ERR_INVALIDATTR:
            win_snprintf (msg, 1000, "Invalid attribute %s for element %s\n",
                     arg1, arg2);
            break;
        case XML_RELAXNG_ERR_LACKDATA:
            win_snprintf (msg, 1000, "Datatype element %s contains no data\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_DATAELEM:
            win_snprintf (msg, 1000, "Datatype element %s has child elements\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_VALELEM:
            win_snprintf (msg, 1000, "Value element %s has child elements\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_LISTELEM:
            win_snprintf (msg, 1000, "List element %s has child elements\n",
                     arg1);
            break;
        case XML_RELAXNG_ERR_DATATYPE:
            win_snprintf (msg, 1000, "Error validating datatype %s\n", arg1);
            break;
        case XML_RELAXNG_ERR_VALUE:
            win_snprintf (msg, 1000, "Error validating value %s\n", arg1);
            break;
    ...
    if (msg[0] == 0) {
        win_snprintf (msg, 1000, "Unknown error code %d\n", err);
    }
    msg[1000 - 1] = 0;
                              ...
                              xmlRelaxNGTypeFree freef)
{
    ...
    if ((xmlRelaxNGRegisteredTypes == NULL) || (_namespace == NULL) ||
        (check == NULL) || (comp == NULL))
        return (-1);
    if (xmlHashLookup(xmlRelaxNGRegisteredTypes, _namespace) != NULL) {
        xmlGenericError(xmlGenericErrorContext,
...
xmlRelaxNGCompile(xmlRelaxNGParserCtxtPtr ctxt, xmlRelaxNGDefinePtr def)
{
    ...
    switch (def->type) {
        ...
        case XML_RELAXNG_VALUE:
            ...
            printf("RNG internal error trying to compile %s\n",
                    xmlRelaxNGDefName(def));
            break;
    }
    return (ret);
...
{
    xmlRelaxNGDefinePtr def = NULL, except_;
...
xmlRelaxNGParseInterleave(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    if (ctxt->interleaves == NULL) {
        ...
        win_snprintf (name, 32, "interleave%d", ctxt->nbInterleaves++);
                       ...
                       xmlRelaxNGParserCtxtPtr ctxt, const xmlChar * name)
{
    ...
    if (choiceOrInterleave == 0) {
        ...
        if (ctxt->interleaves == NULL) {
            ...
            win_snprintf (tmpname, 32, "interleave%d", ctxt->nbInterleaves++);
...
#endif
    if (ctxt == NULL) {
        printf("callback on %s missing context\n", token);
        return;
    }
    if (define == NULL) {
        if (token[0] == '#')
            return;
        printf("callback on %s missing define\n", token);
        if ((ctxt != NULL) && (ctxt->errNo == XML_RELAXNG_OK))
            ctxt->errNo = XML_RELAXNG_ERR_INTERNAL;
        return;
    }
    if ((ctxt == NULL) || (define == NULL)) {
        printf("callback on %s missing info\n", token);
        if ((ctxt != NULL) && (ctxt->errNo == XML_RELAXNG_OK))
            ctxt->errNo = XML_RELAXNG_ERR_INTERNAL;
        return;
    } else if (define->type != XML_RELAXNG_ELEMENT) {
        printf("callback on %s define is not element\n", token);
        if (ctxt->errNo == XML_RELAXNG_OK)
            ctxt->errNo = XML_RELAXNG_ERR_INTERNAL;
        return;
    }
