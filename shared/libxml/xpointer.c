...
xmlXPtrNewPoint(xmlNodePtr node, int indx) {
    ...
    memset (ret, 0 , (unsigned int) sizeof(xmlXPathObject));
...
xmlXPtrLocationSetCreate(xmlXPathObjectPtr val) {
    ...
    memset (ret, 0 , (unsigned int) sizeof(xmlLocationSet));
    if (val != NULL) {
	...
	memset (ret->locTab, 0 ,
	       XML_RANGESET_DEFAULT * (unsigned int) sizeof(xmlXPathObjectPtr));
