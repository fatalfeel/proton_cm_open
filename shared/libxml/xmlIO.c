...
xmlFdWrite (void * context, const char * buffer, int len) 
{
    ...
    if (len > 0) {
	ret = write((int) context, (void*) &buffer[0], len);
...
xmlGzfileOpenW (const char *filename, int compression) {
    ...
    win_snprintf(mode, sizeof(mode), "wb%d", compression);
    if (!strcmp(filename, "-")) {
 ...
 */
static int
xmlZMemBuffExtend( xmlZMemBuffPtr buff, unsigned int ext_amt ) {
...
xmlAllocParserInputBuffer(xmlCharEncoding enc) {
    ...
    memset(ret, 0, (unsigned int) sizeof(xmlParserInputBuffer));
...
xmlAllocOutputBuffer(xmlCharEncodingHandlerPtr encoder) {
    ...
    memset(ret, 0, (unsigned int) sizeof(xmlOutputBuffer));
...
xmlParserInputBufferCreateStatic(const char *mem, int size,
    ...
    memset(ret, 0, (unsigned int) sizeof(xmlParserInputBuffer));
    ret->buffer = xmlBufferCreateStatic((void *)mem, (unsigned int) size);
    if (ret->buffer == NULL) {
        xmlFree(ret);
	return(NULL);
    }
