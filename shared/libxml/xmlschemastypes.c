...
xmlSchemaFormatFloat(double number, char buffer[], int buffersize)
{
	...
	if (buffersize > (int)sizeof("INF"))
	    win_snprintf (buffer, buffersize, "INF");
	break;
    case -1:
	if (buffersize > (int)sizeof("-INF"))
	    win_snprintf (buffer, buffersize, "-INF");
	...
	if (xmlXPathIsNaN(number)) {
		...
		win_snprintf (buffer, buffersize, "NaN");
	} else if (number == 0) {
	    win_snprintf (buffer, buffersize, "0.0E0");
	} else {
	    ...
	    */
	    ...
	    win_snprintf (work, sizeof(work),"%*.*e",
    ...
    *retValue = NULL;
	...
	case XML_SCHEMAS_DECIMAL:
		...
		*retValue = xmlStrdup(BAD_CAST "0.0");
		...
		if (dec.hi != 0)
		    win_snprintf (offs, bufsize - (offs - buf),
	...
	case XML_SCHEMAS_UINT:
		...
		*retValue = (xmlChar*) xmlMalloc(bufsize);
		...
		if (dec.hi != 0) {
		    if (dec.sign)
			win_snprintf ((char *) *retValue, bufsize,
			    "-%lu%lu%lu", dec.hi, dec.mi, dec.lo);
		    else
			win_snprintf ((char *) *retValue, bufsize,
			    "%lu%lu%lu", dec.hi, dec.mi, dec.lo);
		    ...
		    if (dec.sign)
			win_snprintf ((char *) *retValue, bufsize, "-%lu", dec.lo);
		    else
			win_snprintf ((char *) *retValue, bufsize, "%lu", dec.lo);
		}
	    }
	...
	case XML_SCHEMAS_DURATION: {
		...
		*/
		year = (unsigned long) FQUOTIENT(abs(val->value.dur.mon), 12);
		mon = abs(val->value.dur.mon) - 12 * year;
		...
		if ((val->value.dur.mon < 0) || (val->value.dur.sec < 0))
		    win_snprintf (buf, 100, "P%luY%luM%luDT%luH%luM%.14gS",
	...
	case XML_SCHEMAS_GYEAR: {
		...
		win_snprintf (buf, 30, "%04ld", val->value.date.year);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;
	case XML_SCHEMAS_GMONTH: {
		...
		*retValue = (xmlChar*) xmlMalloc(6);
		if (*retValue == NULL)
		    return(-1);
		win_snprintf ((char *) *retValue, 6, "--%02u",
		    val->value.date.mon);
	    }
	    break;
        case XML_SCHEMAS_GDAY: {
		...
		*retValue = (xmlChar*) xmlMalloc(8);
		if (*retValue == NULL)
		    return(-1);
		win_snprintf ((char *) *retValue, 8, "--%02u-%02u",
		...
		if (val->value.date.year < 0)
		    win_snprintf (buf, 35, "-%04ld-%02u",
			abs(val->value.date.year), 
			val->value.date.mon);
		else
		    win_snprintf (buf, 35, "%04ld-%02u",
	...
	case XML_SCHEMAS_FLOAT: {
		...
		* the whole range.
		*/
		win_snprintf (buf, 30, "%01.14e", val->value.f);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;
	case XML_SCHEMAS_DOUBLE: {
		...
		* the whole range.
		*/
		win_snprintf (buf, 40, "%01.14e", val->value.d);
		*retValue = BAD_CAST xmlStrdup(BAD_CAST buf);
	    }
	    break;	
	default:
