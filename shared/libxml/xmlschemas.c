...
#define FREE_AND_KD_NULL(str) if ((str) != NULL) { xmlFree((xmlChar *) (str)); str = NULL; }
	...
	*/
	if ((facetType == XML_SCHEMA_FACET_LENGTH) ||
	    ...
	    */
	    ...
	    win_snprintf (len, 24, "%lu", xmlSchemaGetFacetValueAsULong(facet));
	    win_snprintf (actLen, 24, "%lu", length);
                         ...
                         xmlNodePtr node, int topLevel)
{
	...
	*/
#ifdef ENABLE_NAMED_LOCALS
        win_snprintf (buf, 39, "#ST%d", ctxt->counter++ + 1);
                          ...
                          xmlNodePtr node, int topLevel)
{
	...
	*/
#ifdef ENABLE_NAMED_LOCALS
        win_snprintf (buf, 39, "#CT%d", ctxt->counter++ + 1);
	type = xmlSchemaAddType(ctxt, schema,
			  ...
			  xmlSchemaTypePtr type)
{
	...
	*/
	if ((WXS_IS_COMPLEX(baseType)) &&
	    ...
	    */
#ifdef ENABLE_NAMED_LOCALS
	    win_snprintf (buf, 29, "#scST%d", ++(pctxt->counter));
				  ...
				  xmlSchemaAttributeGroupPtr attrGr)
{
	...
	&(attrGr->attributeWildcard), (xmlSchemaItemListPtr) attrGr->attrUses, NULL) == -1)
	return(-1);
    return(0);
}
	...
	if (iattr->state == XML_SCHEMAS_ATTR_DEFAULT) {
		...
		*/
		...
		if (iattr->nsName == NULL) {
			...
			*/
			do {
			    win_snprintf ((char *) prefix, 12, "p%d", counter++);
