...
xmlXPathFormatNumber(double number, char buffer[], int buffersize)
{
	...
	if (buffersize > (int)sizeof("Infinity"))
	    win_snprintf (buffer, buffersize, "Infinity");
	break;
    case -1:
	if (buffersize > (int)sizeof("-Infinity"))
	    win_snprintf (buffer, buffersize, "-Infinity");
	...
	if (xmlXPathIsNaN(number)) {
	    ...
	    win_snprintf (buffer, buffersize, "0");
	    ...
	    */
		...
		size = win_snprintf (work, sizeof(work),"%*.*e",
		...
		size = win_snprintf (work, sizeof(work), "%0.*f",
