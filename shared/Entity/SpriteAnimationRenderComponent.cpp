...
{
	m_pSpriteAnimationSet = ResourceManager::GetResourceManager()->GetSpriteAnimationSet(pFileName->GetString());

	if (m_pSpriteAnimationSet)
	{
		m_pSpriteSheet = ResourceManager::GetResourceManager()->GetSurfaceResource<SpriteSheetSurface>(m_pSpriteAnimationSet->GetSpriteSheetImageName());
	}
...
void SpriteAnimationRenderComponent::OnAdd(Entity *pEnt)
{
	...
	GetParent()->GetFunction("OnRender")->m_sig_function.connect(1, boost::bind(&SpriteAnimationRenderComponent::OnRender, this, _1));
}
