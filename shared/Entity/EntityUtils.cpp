...
/*EntityComponent* SetupInterpolateComponent(Entity *pEnt, const string& componentName, const string& varName, const Variant& targetValue, int durationMS, int delayBeforeStartMS, eInterpolateType interpolationType = INTERPOLATE_SMOOTHSTEP, InterpolateComponent::eOnFinish onFinish = InterpolateComponent::ON_FINISH_DIE, eTimingSystem timing = GetTiming())
{
	...
	} else
	{
		...
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayBeforeStartMS, "duration_ms", Variant(uint32(durationMS)), timing);
	}
...
void AnimateStopEntity(Entity *pEnt, int delayToStartMS)
{
	...
	if (pComp)
	{
		...
		{
			MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "duration_ms", Variant(uint32(0)));
		}
	}
}
...
void AnimateStopEntityAndSetFrame(Entity *pEnt, int delayToStartMS, int frameX, int frameY)
{
	...
	if (pComp)
	{
		...
		{
			MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "frameX", Variant(uint32(frameX)));
			MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "frameY", Variant(uint32(frameY)));
		}
	}
}
...
EntityComponent * SetupAnimEntity(Entity *pEnt, uint32 frameCountX, uint32 frameCountY, int curFrameX, int curFrameY)
{
	...
	if (pComp)
	{
		...
		pComp->GetFunction("SetupAnim")->m_sig_function(&vList);
...
/*void AnimateEntity(Entity *pEnt, int startFrame, int endFrame, int animSpeedMS, InterpolateComponent::eOnFinish type, int delayToStartMS)
{
	...
	} else
	{
		...
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "component_name", Variant("OverlayRender"));
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "var_name", Variant(frameName));
		MessageManager::GetMessageManager()->SetComponentVariable(overlayComp, delayToStartMS, frameName, Variant(uint32(startFrame)));
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "target", Variant(uint32(endFrame + 1)));
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "set_value_on_finish", Variant(uint32(endFrame)));
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "interpolation", Variant(uint32(INTERPOLATE_LINEAR)));
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "on_finish", Variant(uint32(type)));
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayToStartMS, "duration_ms", Variant(uint32(totalTimeMS)));
	}
}*/
...
EntityComponent * SetOverlayImageEntity(Entity *pEntWithOverlayComponent, string imageFileName, uint32 delayBeforeActionMS, bool bAddBasePath)
{
	...
	} else
	{
		...
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayBeforeActionMS, "fileName", imageFileName);
	}
	return pComp;
}
...
void EnableAllButtonsEntity(Entity *pEnt, bool bRecursive, int delayBeforeActionMS, eTimingSystem timing)
{
	...
	if (pComp)
	{
		...
		{
			//schedule it instead
			MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayBeforeActionMS,"disabled",uint32(0), timing);
		}
	}
...
void KillEntity(Entity *pEnt, int timeMS, eTimingSystem timingSystem)
{
	...
	} else
	{
		...
		MessageManager::GetMessageManager()->CallEntityFunction(pEnt, timeMS, "OnDelete", &vList, timingSystem);
	}
}
...
/*Entity * CreateInputTextEntity(Entity *pParentEnt, string name, float x, float y, string text, float sizeX, float sizeY)
{
	...
	float fontHeight = BaseApp::GetBaseApp()->GetFont(FONT_SMALL)->GetLineHeight(1.0f);
...
void AddFocusIfNeeded(Entity *pEnt, bool bAlsoLinkMoveMessages, int delayInputMS, int updateAndRenderDelay)
{
	...
	if (!pComp)
	{
		...
		{
			//schedule it
			MessageManager::GetMessageManager()->AddComponent(pEnt, updateAndRenderDelay, new FocusUpdateComponent);
	...
	if (!pEnt->GetComponentByName("FocusRender", true))
	{
		...
		{
			//schedule it
			MessageManager::GetMessageManager()->AddComponent(pEnt, updateAndRenderDelay, new FocusRenderComponent);
...
void EnableRawMessageFocusInput(Entity *pEnt)
{
	...
	if (pInputComp)
	{
		pInputComp->GetFunction("LinkRawMessages")->m_sig_function(NULL);
	}
}
...
/*void GetFontAndScaleToFitThisStringInWidthPixels(eFont *pFontIDOut, float *pFontScaleOut, string text, float desiredWidth)
{
	*pFontIDOut = FONT_SMALL;
	float sizeY = BaseApp::GetBaseApp()->GetFont(*pFontIDOut)->GetLineHeight(1.0f);

	*pFontScaleOut = 1.0f;

	CL_Vec2f vSize = BaseApp::GetBaseApp()->GetFont(*pFontIDOut)->MeasureText(text, *pFontScaleOut);
	...
	*pFontIDOut = FONT_LARGE;
	...
	vSize = BaseApp::GetBaseApp()->GetFont(*pFontIDOut)->MeasureText(text, *pFontScaleOut);
	*pFontScaleOut = desiredWidth/vSize.x;
}
...
void GetFontAndScaleToFitThisLinesPerScreenY(eFont *pFontIDOut, float *pFontScaleOut, float desiredLinesPerScreenY)
{
	...
	*pFontScaleOut = (GetScreenSizeYf()/desiredLinesPerScreenY)/sizeY;
	...
	//let's use the bigger font instead
	*pFontIDOut = FONT_LARGE;
	sizeY = BaseApp::GetBaseApp()->GetFont(*pFontIDOut)->GetLineHeight(1.0f);
	*pFontScaleOut = (GetScreenSizeYf()/desiredLinesPerScreenY)/sizeY;
}
...
{
	float fontSizeRating = GetScreenSizeYf() / BaseApp::GetBaseApp()->GetFont(fontID)->GetLineHeight(fontScale);
...
/*Entity * CreateScrollingTextBoxEntity(Entity *pParent, string entName, CL_Vec2f vPos, CL_Vec2f vTextAreaSize, string msg, float scale, eAlignment textAlignment)
{
	...
	pScroll->GetFunction("ResizeScrollBounds")->m_sig_function.connect(ResizeScrollBounds);
	...
	MessageManager::GetMessageManager()->CallEntityFunction(pScroll, 1, "ResizeScrollBounds", &vList);

	//also, let's have it resize itself whenever the text changes
	pText->GetFunction("OnSizeChanged")->m_sig_function.connect(ResizeScrollerAfterTextboxChanges);
	return pText;
}*/
									  ...
									  TyperComponent::eMode mode, string textToAddByTyping)
{
	...
	if (delayBeforeActionMS != 0)
	{
		...
		MessageManager::GetMessageManager()->SetComponentVariable(pTextTyper, delayBeforeActionMS, "paused", Variant(uint32(0)));
	}
...
void PreloadKeyboard(OSMessage::eParmKeyboardType keyboardType)
{
    ...
    BaseApp::GetBaseApp()->m_sig_hardware(&v);
...
void SendFakeInputMessageToEntity(Entity *pEnt, eMessageType msg, CL_Vec2f vClickPos, int delayBeforeStartingMS)
{
	...
	if (delayBeforeStartingMS == 0)
	{
		...
		BaseApp::GetBaseApp()->GetTouch(C_MAX_TOUCHES_AT_ONCE-1)->SetPos(vClickPos);
	...
	{
		//schedule it to happen later
		MessageManager::GetMessageManager()->CallEntityFunction(pEnt, delayBeforeStartingMS, "OnInput", &v);
...
void SendFakeButtonPushToEntity(Entity *pEntity, int timeMS)
{
	...
	MessageManager::GetMessageManager()->CallEntityFunction(pEntity, timeMS, "OnButtonSelected", &vList);
}
...
/*void SetupLightBarSelect(Entity *pBG, string entNamePrefix, int defaultOption, uint32 color)
{
	...
	for (int i=0;;i++)
	{
		...
		if (pEnt)
		{
			...
			pEnt->GetFunction("OnButtonSelected")->m_sig_function.connect(&LightBarOnChange);
...
void SetScrollProgressEntity(Entity *pEnt, const CL_Vec2f &progress)
{
	...
	pComp->GetFunction("SetProgress")->m_sig_function(&scrollProgress);
}
...
EntityComponent * DisableComponentByName(Entity *pEnt, const string &compName, int delayBeforeActionMS)
{
	...
	if (delayBeforeActionMS != 0)
	{
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayBeforeActionMS, "disabled", Variant(uint32(1)));
	} else
	{
		//set it now
		pComp->GetVar("disabled")->Set(uint32(1));
...
EntityComponent * EnableComponentByName(Entity *pEnt, const string &compName, int delayBeforeActionMS)
{
	...
	if (delayBeforeActionMS != 0)
	{
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, delayBeforeActionMS, "disabled", Variant(uint32(0)));
	} else
	{
		//set it now
		pComp->GetVar("disabled")->Set(uint32(0));
...
void DestroyUnusedTextures()
{
	...
	GetUsedTextures(usedTextures, Entity::GetEntityManager());
#ifdef _DEBUG
	...
	*/
	...
	ResourceManager::GetResourceManager()->RemoveTexturesNotInExclusionList(usedTextures);
}
...
EntityComponent * CreateSlider(Entity *pBG, float x, float y, float sizeX, string buttonFileName, string left, string middle, string right)
{
	...
	float textY = y- (BaseApp::GetBaseApp()->GetFont(FONT_SMALL)->GetLineHeight(1)+iPhoneMapY2X(15));
...
Entity * CreateCheckbox(Entity *pBG, string name, string text, float x, float y, bool bChecked, eFont fontID, float fontScale)
{
	...
	SurfaceAnim *pAnim = ResourceManager::GetResourceManager()->GetSurfaceAnim("interface/checkbox.rttex");
	...
	float letterHeight = BaseApp::GetBaseApp()->GetFont(fontID)->MeasureText("W", fontScale).y;
	...
	pEnt->GetFunction("OnButtonSelected")->m_sig_function.connect(&OnCheckboxToggle);
...
/*void OnShowTextMessage(VariantList *pVList)
{
	...
	VariantList v(pRect);
	MessageManager::GetMessageManager()->CallEntityFunction(pRect, timeMS+1, "MoveToTop", &v, TIMER_SYSTEM);
	pRect->GetFunction("MoveToTop")->m_sig_function.connect(MoveEntityToTop);
...
void ShowTextMessage(string msg, int timeMS, int delayBeforeStartingMS)
{
	...
	MessageManager::GetMessageManager()->CallStaticFunction(OnShowTextMessage, delayBeforeStartingMS, &vList, TIMER_SYSTEM);
}*/
...
void ActivateTextInputEntity(Entity *pEnt)
{
	...
	if (pEnt)
	{
		pInputComp->GetFunction("ActivateKeyboard")->m_sig_function(NULL);
	}
}
