...
void SpriteAnimationUtils::StartAnimationEntity(Entity *pEntity, const std::string& animationName, unsigned int animationDuration, unsigned int delayToStartMS, InterpolateComponent::eOnFinish finishType)
{
	...
	{
		MessageManager::GetMessageManager()->SetComponentVariable(spriteAnimComp, delayToStartMS, "animationName", Variant(animationName));
		MessageManager::GetMessageManager()->SetComponentVariable(animator, delayToStartMS, "duration_ms", Variant(uint32(animationDuration)));
	}
}
