...
void UnpackArchiveComponent::OnAdd(Entity *pEnt)
{
	...
	*/
	...
	GetParent()->GetFunction("OnUpdate")->m_sig_function.connect(1, boost::bind(&UnpackArchiveComponent::OnUpdate, this, _1));
}
	...
	switch (m_tarHandler.GetState())
	{
		...
		case TarHandler::STATE_BZIPPING:
			...
			if (m_tarHandler.ProcessChunk())
			{
				...
				GetFunction("OnStatusUpdate")->m_sig_function(&vList);
			}
