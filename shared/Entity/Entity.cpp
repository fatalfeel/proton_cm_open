...
static Entity* entity_manager_instance = NULL;

Entity* Entity::GetEntityManager()
{
	if (!entity_manager_instance)
		entity_manager_instance = new Entity;
...
void Entity::Free()
{
	if( entity_manager_instance )
	{
		delete entity_manager_instance;
...
void Entity::OneTimeInit()
{
	...
	GetFunction("OnDelete")->m_sig_function.connect(1, boost::bind(&Entity::OnDelete, this, _1));
}
...
void Entity::CallFunctionRecursivelyWithUpdatedVar( const string funcName, VariantList *pVList, const string &varName, int varIndex, eRecursiveVarOp op, vector<EntityCall> *pEntList )
{
	...
	if (pFilterFunc)
	{
		...
		pFilterFunc->m_sig_function(pVList);
			...
			case Variant::TYPE_VECTOR2:
					switch (op)
					{
						...
						case RECURSIVE_VAR_OP_SUBTRACTION_PLUS_ALIGNMENT_OFFSET:
							...
							break;
						default:
							LogError("CallFunctionRecursivelyWithUpdatedVar: Bad op");
							assert(0);
							break;
					}		
					break;
...
EntityComponent * Entity::GetComponentByName( string const &name, bool bAlsoCheckParents )
{
	...
	if (!bAlsoCheckParents || !GetParent()) 
		return NULL;
...
void Entity::SetTaggedForDeletion()
{
	...
	MessageManager::GetMessageManager()->CallEntityFunction(this, 0, "OnDelete", &vList);
}
