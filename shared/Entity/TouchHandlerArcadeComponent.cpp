...
void TouchHandlerArcadeComponent::OnAdd(Entity *pEnt)
{
	...
	GetParent()->GetFunction("OnInput")->m_sig_function.connect(1, boost::bind(&TouchHandlerArcadeComponent::OnInput, this, _1));
	//GetFunction("EndClick")->m_sig_function.connect(1, boost::bind(&TouchHandlerArcadeComponent::EndClick, this, _1));
...
void TouchHandlerArcadeComponent::HandleClickStart(CL_Vec2f &pt, uint32 fingerID)
{
	...
	if (m_touchArea.contains(pt))
	{
		...
		GetParent()->GetFunction("OnTouchStart")->m_sig_function(&vList);
...
void TouchHandlerArcadeComponent::HandleClickEnd( CL_Vec2f &pt, uint32 fingerID )
{
	...
	{
		GetParent()->GetFunction("OnTouchEnd")->m_sig_function(&vList);
	}
...
void TouchHandlerArcadeComponent::EndPinchIfNeeded()
{
	...
	{
		m_bIsPinching = false;
		GetParent()->GetFunction("OnPinchEnd")->m_sig_function(NULL);
	}
}
...
void TouchHandlerArcadeComponent::HandleClickMoveSecond( CL_Vec2f &pt, uint32 fingerID )
{
	...
	if (m_bIsPinching || fabs(finalMod) > C_PERCENT_OF_PINCH_TO_DETECT_IT)
	{
		...
		{
			m_bIsPinching = true;
			GetParent()->GetFunction("OnPinchStart")->m_sig_function(NULL);
		}
		
		VariantList vList(GetParent(), -finalMod);
		GetParent()->GetFunction("OnPinchMod")->m_sig_function(&vList);
