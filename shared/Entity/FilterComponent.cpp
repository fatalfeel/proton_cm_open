...
void FilterComponent::OnAdd(Entity *pEnt)
{
	...
	GetParent()->GetFunction(string("Filter")+*m_pFunctionName)->m_sig_function.connect(1, boost::bind(&FilterComponent::FilterOnInput, this, _1));
}
...
void SetFilterSetting( Entity *pEnt, string functionName, Entity::eFilterCommunication filterSetting, int timeMS /*= 0*/ )
{
	...
	if (timeMS)
	{
		...
		MessageManager::GetMessageManager()->SetComponentVariable(pComp, timeMS, "filterSetting", uint32(filterSetting));
	} else
	{
