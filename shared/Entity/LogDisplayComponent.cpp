...
void LogDisplayComponent::OnAdd(Entity *pEnt)
{
	...
	if (!m_pActiveConsole) 
	{
		m_pActiveConsole = BaseApp::GetBaseApp()->GetConsole();
		m_pActiveConsole->m_sig_on_text_added.connect(boost::bind(&LogDisplayComponent::OnTextAdded, this));

	}
	//register ourselves to render if the parent does
	GetParent()->GetFunction("OnRender")->m_sig_function.connect(1, boost::bind(&LogDisplayComponent::OnRender, this, _1));
	GetFunction("AddLine")->m_sig_function.connect(1, boost::bind(&LogDisplayComponent::AddLine, this, _1));
	GetParent()->GetFunction("OnUpdate")->m_sig_function.connect(1, boost::bind(&LogDisplayComponent::OnUpdate, this, _1));
...
void LogDisplayComponent::AddLine(VariantList *pVList)
{
	...
	BaseApp::GetBaseApp()->GetFont(eFont(*m_pFontID))->MeasureTextAndAddByLinesIntoDeque(*m_pSize2d, pVList->Get(0).GetString(), &lines, *m_pFontScale, enclosedSize2d);
...
void LogDisplayComponent::OnEnableScrollingChanged(Variant *pVariant)
{
	...
	pDragComp->GetFunction("OnTouchDragUpdate")->m_sig_function.connect(1, boost::bind(&LogDisplayComponent::OnTouchDragUpdate, this, _1));	
	GetParent()->GetFunction("OnOverStart")->m_sig_function.connect(1, boost::bind(&LogDisplayComponent::OnOverStart, this, _1));	
	GetParent()->GetFunction("OnOverEnd")->m_sig_function.connect(1, boost::bind(&LogDisplayComponent::OnOverEnd, this, _1));	
...
void LogDisplayComponent::OnUpdate(VariantList *pVList)
{
	if (m_pScrollBarComp)
	{
		...
		ModByDistance(-(m_vecDisplacement.y*BaseApp::GetBaseApp()->GetDelta()));
		m_vecDisplacement *= (1- (*m_pFriction*BaseApp::GetBaseApp()->GetDelta()));
	}
}

void LogDisplayComponent::ModByDistance(float mod)
{
	RTFont *pFont = BaseApp::GetBaseApp()->GetFont(eFont(*m_pFontID));
...
void LogDisplayComponent::UpdateScrollBar()
{
	...
	RTFont *pFont = BaseApp::GetBaseApp()->GetFont(fontID);
	float fontHeight = pFont->GetLineHeight(*m_pFontScale);
	float linePerScreen =  (vTotalBounds.get_height()/fontHeight);
...
{
	Entity *pConsole = Entity::GetEntityManager()->GetEntityByName("ConsoleEnt");
	...
	{
		pConsole = Entity::GetEntityManager()->AddEntity(new Entity("ConsoleEnt"));
