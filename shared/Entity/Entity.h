...
{
public:
	static Entity* GetEntityManager();
	static void Free();
	 ...
	 */
	Variant * GetVar(const string &varName) 
	{
	 ...
	 */
	Variant * GetVarWithDefault(const string &varName, const Variant &var) 
	{
		return m_sharedDB.GetVarWithDefault(varName, var);
	}
	
	FunctionObject * GetFunction(const string &funcName) 
	{
	...
	bool GetTaggedForDeletion() 
	{
		return m_bTaggedForDeletion;
	}
