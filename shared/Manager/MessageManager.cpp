...
void Message::SetTargetComponent( EntityComponent *pComp )
{
	...
	m_pComponent = pComp;
	if (!pComp) return;
	m_pComponent->GetFunction("OnDelete")->m_sig_function.connect(1, boost::bind(&Message::OnComponentDestroyed, this, _1));
}
...
//by jesse stone
static MessageManager* message_manager_instance = NULL;

MessageManager* MessageManager::GetMessageManager()
{
	if (!message_manager_instance)
		message_manager_instance = new MessageManager;
	return message_manager_instance;
}
...
void MessageManager::Free()
{
	if( message_manager_instance )
	{
		delete message_manager_instance;
		message_manager_instance = NULL;
	}
}
