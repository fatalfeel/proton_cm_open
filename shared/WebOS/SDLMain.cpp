...
void IAPBuyItem(string m_item)
{
	...
	if (GetDeviceOSVersion() < 3.0f)
	{
		...
		MessageManager::GetMessageManager()->SendGUI(MESSAGE_TYPE_IAP_RESULT,(float)IAPManager::RESULT_SERVICE_UNAVAILABLE,0.0f);
		return;
	}
	...
	if (!itemReceipt || !itemReceiptJSON)
	{
		...
		MessageManager::GetMessageManager()->SendGUI(MESSAGE_TYPE_IAP_RESULT,(float)IAPManager::RESULT_DEVELOPER_ERROR,0.0f);
		return;
	}
	...
	if (CaseInsensitiveCompare(receiptStatus, "Charged"))
	{
		...
		MessageManager::GetMessageManager()->SendGUIStringEx(MESSAGE_TYPE_IAP_RESULT,(float)IAPManager::RESULT_OK,0.0f,0, extra);
	} else
	{
		MessageManager::GetMessageManager()->SendGUI(MESSAGE_TYPE_IAP_RESULT,(float)IAPManager::RESULT_USER_CANCELED,0.0f);
	}
		...
		case SDL_JOYAXISMOTION:
			{
				...
				MessageManager::GetMessageManager()->SendGUI(MESSAGE_TYPE_GUI_ACCELEROMETER, Variant(g_accelHold));
	...
	case SDL_ACTIVEEVENT:
 	...
 	break;
		case SDL_MOUSEBUTTONDOWN:
			...
			MessageManager::GetMessageManager()->SendGUIEx(MESSAGE_TYPE_GUI_CLICK_START, xPos, yPos, ev.motion.which);
			}
			break;
		case SDL_MOUSEBUTTONUP:
			{
				...
				float yPos = ev.motion.y;
				ConvertCoordinatesIfRequired(xPos, yPos);
				MessageManager::GetMessageManager()->SendGUIEx(MESSAGE_TYPE_GUI_CLICK_END, xPos, yPos, ev.motion.which);
			}
			break;
		...
		case SDL_MOUSEMOTION:     
			{
				...
				{
					MessageManager::GetMessageManager()->SendGUIEx(MESSAGE_TYPE_GUI_CLICK_MOVE, xPos, yPos, ev.motion.which);
					break;
				} 

#ifdef WIN32
				MessageManager::GetMessageManager()->SendGUIEx(MESSAGE_TYPE_GUI_CLICK_MOVE_RAW, xPos, yPos, ev.motion.which);
#endif
			}
